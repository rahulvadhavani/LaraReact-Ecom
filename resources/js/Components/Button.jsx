...
export default ({
    ...
    switch (variant) {
        ...
        case 'info':
            ...
            break
        case 'black':
            variantClasses = `mx-2 px-4 py-2 bg-gray-800 dark:text-gray-900 dark:bg-gray-100 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150 false`
            break
        default:
            variantClasses = `bg-purple-500 text-white hover:bg-purple-600 focus:ring-purple-500`
    }

    const sizeClasses = `${size == 'base' ? (iconOnly ? 'p-2' : 'px-4 py-2 text-base') : ''
        } ${size == 'sm' ? (iconOnly ? 'p-1.5' : 'px-2.5 py-1.5 text-sm') : ''} ${size == 'lg' ? (iconOnly ? 'p-3' : 'px-5 py-2 text-xl') : ''
        }`

    const roundedClasses = `${!squared && !pill ? 'rounded-md' : ''} ${pill ? 'rounded-full' : ''
        }`

    const iconSizeClasses = `${size == 'sm' ? 'w-5 h-5' : ''} ${size == 'base' ? 'w-6 h-6' : ''
        } ${size == 'lg' ? 'w-7 h-7' : ''}`
    ...
    if (href) {
        ...
        return (
            <Tag
                target={target}
                href={href}
                className={`${baseClasses} ${sizeClasses} ${variantClasses} ${roundedClasses} ${className} ${processing ? 'pointer-events-none opacity-50' : ''
                    }`}
            >
                {children}
